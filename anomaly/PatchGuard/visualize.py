...
def get_heatmap(raw_image, localization):
    ano_map = gaussian_filter(localization, sigma=8)
        ...
        for images, _, masks, _ in test_loader:
            ...
            for mode in ["clean"]: # "clean", "adv"
                ...
                for b in range(batch_size):
                    ...
                    localization = gaussian_filter(localization, sigma=8)
                    # localization = min_max_norm(localization)

                    print(f"Max Score: {localization.max():.4f}")
                    ...
                    fig, ax = plt.subplots(1, 1, figsize=(5, 5))
                    ax.imshow(images[b].cpu().detach().permute(1, 2, 0).numpy())
                    ax.imshow(localization, cmap=cmap, interpolation='bilinear', vmin=0, vmax=0.8) # lcd : vmin=-3.3, vmax=-3.0, sigma=8 // printer : vmin=0, vmax=0.8, sigma=8 
                    ax.axis('off')
