...
class MVTec(data.Dataset):
    def __init__(self, path, class_name, transform=None, mask_transform=None, seed=0, split='train', size=256):
    ...
    def __getitem__(self, idx):
        ...
        if mask_path:
            if os.path.exists(mask_path):
                ...
                # 마스크가 없으면 그냥 256x256 검은색 이미지 생성 (학습 이미지 사이즈에 맞게 조절)
                mask = Image.new('RGB', (768, 768), (0, 0, 0))
            # mask = Image.open(mask_path).convert('RGB')
